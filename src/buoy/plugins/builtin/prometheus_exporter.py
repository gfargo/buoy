...
class PrometheusExporterPlugin(Plugin):
    ...
    def format_metrics(stats: dict) -> str:
        ...
        lines.append(f'buoy_cpu_percent{{host="{stats.get("hostname", "")}"}} {stats.get("cpu", 0)}')
        ...
        lines.append(f'buoy_memory_total_bytes{{host="{stats.get("hostname", "")}"}} {mem_total_bytes}')

        lines.append("# HELP buoy_temperature_celsius CPU temperature")
        lines.append("# TYPE buoy_temperature_celsius gauge")
        lines.append(f'buoy_temperature_celsius{{host="{stats.get("hostname", "")}"}} {stats.get("temp", 0)}')

        lines.append("# HELP buoy_disk_used_percent Root disk usage percentage")
        lines.append("# TYPE buoy_disk_used_percent gauge")
        lines.append(f'buoy_disk_used_percent{{host="{stats.get("hostname", "")}"}} {stats.get("disk_pct", 0)}')
        ...
        lines.append(f'buoy_containers_running{{host="{stats.get("hostname", "")}"}} {stats.get("containers", 0)}')
        ...
        if nvme:
            ...
            lines.append(f'buoy_nvme_temperature_celsius{{host="{stats.get("hostname", "")}"}} {nvme.get("temp", 0)}')

            lines.append("# HELP buoy_nvme_wear_percent NVMe wear percentage")
            lines.append("# TYPE buoy_nvme_wear_percent gauge")
            lines.append(f'buoy_nvme_wear_percent{{host="{stats.get("hostname", "")}"}} {nvme.get("wear_pct", 0)}')
