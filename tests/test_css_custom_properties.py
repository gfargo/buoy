...
Scans:
  - static/css/buoy.css           (core stylesheet, var() usages)
...
THEMES_DIR = REPO_ROOT / "static" / "css" / "themes"
CORE_CSS = REPO_ROOT / "static" / "css" / "buoy.css"
...
def _all_referenced_vars() -> dict[str, set[str]]:
    ...
    for var in _collect_referenced(CORE_CSS):
        _add(var, CORE_CSS.name)
