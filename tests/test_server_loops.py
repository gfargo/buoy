...
def _make_config():
    ...
    config.refresh = RefreshConfig(
        stats_interval=0, fleet_interval=0, image_updates_interval=0, health_check_interval=0
    )
    return config
...
class TestHealthCheckLoopLogging:
    @pytest.mark.asyncio
    async def test_initial_check_failure_is_logged(self, caplog):
        state = BuoyAppState(config=_make_config())
        ...
        with caplog.at_level(logging.WARNING, logger="buoy.server"):
            with pytest.raises(TimeoutError):
                await asyncio.wait_for(
                    _health_check_loop(state, _ExplodingHealthChecker()), timeout=0.2
                )
